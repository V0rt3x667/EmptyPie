...
datadir="${home}/EmptyPie"
biosdir="${datadir}/bios"
romdir="${datadir}/roms"
emdatdir="${datadir}/configs"
...
if [[ "$(id -u)" -ne 0 ]]; then
    echo "The EmptyPie setup script must be run under sudo. Try 'sudo ${0}'"
    exit 1
fi

__backtitle="EmptyPie Setup - Installation Folder: ${rootdir} User: ${__user}"

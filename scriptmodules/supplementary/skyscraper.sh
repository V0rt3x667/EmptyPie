...
function sources_skyscraper() {
    ...
    for file in "${files[@]}"; do
        sed -e 's|RetroPie|EmptyPie|g' -i "${md_build}/${file}"
        sed -e 's|retropie|emptypie|g' -i "${md_build}/${file}"
    done
}
...
function _scrape_skyscraper() {
    ...
    # Trap 'ctrl+c' & Return If Pressed Rather Than Exiting EmptyPie-Setup

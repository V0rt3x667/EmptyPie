...
function rps_retronet_nickname() {
    cmd=(dialog --backtitle "${__backtitle}" --inputbox "Please Enter The Nickname You Wish To Use (Default: EmptyPie)" 22 76 "${__netplaynickname}")
    choice=$("${cmd[@]}" 2>&1 >/dev/tty)
    if [[ -n "${choice}" ]]; then

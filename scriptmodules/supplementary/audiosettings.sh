...
EOF
    ...
    local dest="/etc/alsa/conf.d/99-emptypie.conf"
    mkdir -p /etc/alsa/conf.d
    mv "${tmpfile}" "${dest}"
    chmod 644 "${dest}"
}
    ...
    if [[ -n "${choice}" ]]; then
        case "${choice}" in
            [0-9]*)
                ...
                rm -f "/etc/alsa/conf.d/99-emptypie.conf"

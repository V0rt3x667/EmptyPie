...
rp_module_id="setup"
rp_module_desc="Setup GUI for EmptyPie"
rp_module_section=""
...
function rps_logStart() {
    ...
    echo "EmptyPie-Setup Version: ${__version} ($(sudo -u "${__user}" git -C "${scriptdir}" log -1 --pretty=format:%h))"
...
function updatescript_setup() {
    ...
    printHeading "Fetching the latest version of the EmptyPie Setup Script."
    pushd "$scriptdir" >/dev/null || exit
    if [[ ! -d ".git" ]]; then
        printMsgs "dialog" "Cannot find directory '.git'. Please clone the EmptyPie Setup Script via 'git clone https://github.com/V0rt3x667/EmptyPie-Setup.git'"
        popd >/dev/null || exit
        return 1
    fi
    local error
    ...
    printMsgs "dialog" "Fetched the latest version of the EmptyPie Setup Script."
    return 0
}
            ...
            X)
                ...
                case "${__mod_info[${id}/section]}" in
                    core)
                        ...
                        ;;
                    depends)
                        text+="\n\nWARNING! - This package is required by other EmptyPie packages, removing may cause other packages to fail."
...
function uninstall_setup()
{
    ...
    printMsgs "dialog" "EmptyPie has been uninstalled."
}
...
function gui_setup() {
    ...
    while true; do
        ...
        options=(
            ...
            S "Update EmptyPie-Setup Script"
            "S Update the EmptyPie-Setup script. This will update the main management script only, but will not update any software packages. To update packages use the 'Update' option from the main menu, which will also update the EmptyPie-Setup script."

            X "Uninstall EmptyPie"
            "X Uninstall EmptyPie completely."

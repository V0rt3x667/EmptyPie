...
## @file helpers.sh
## @brief EmptyPie helpers library
## @copyright GPLv3
...
## @fn hasPackage()
## @param package name of Void Linux package
...
## @brief Test for an installed Void Linux package / package version.
...
function hasPackage() {
    ...
    local ver
    local status
    local out="$(xbps-query -p pkgver "${pkg}" &>/dev/null)"
    if [[ "${?}" -eq 0 ]]; then
        ver="${out##*-}"
        status="Installed"
    fi
...
## Helper function to manage DKMS modules installed by EmptyPie
...
function adminRsync() {
    ...
    [[ -z "$remote_user" ]] && remote_user="emptypie"
